...
class DynaConfigMixin:
    """
        Used to parse the schedule, allocator and aggregator types from the config file.
    """
    ...
    def __parse_schedule__(self, schedule_type: str) -> BaseSchedule:
        ...
        if schedule == 'no_schedule':
            ...
            warnings.warn(
                'No arguments provided for schedule. Using no_schedule as default.')
            return BaseSchedule()
        elif schedule == 'once':
            ...
            warnings.warn(
                'Invalid schedule type. Using no_schedule as default.')
            return BaseSchedule()
    ...
    def __parse_allocator__(self, allocator_type: str) -> BaseAllocator:
        ...
        if len(args) == 0:
            warnings.warn(
                'No arguments provided for allocator. Using topk;1 as default.')
...
@dataclass
class DynaLoraConfig(DynaConfigMixin, LoraConfig):
    """
...
@dataclass
class DynaVeraConfig(DynaConfigMixin, VeraConfig):
    """
        Configuration for the dynamic LoRA model.
        ...
        Currently, only 'l2' and 'l1' are supported.
    """
    # we have to add these here unfortunately
    schedule_type: str = field(
        default='no_schedule',
        metadata={'help': ("The schedule type that will determine how often the adapters are updated. By default, only once in the beginning. Choices: ['no_schedule', 'once;<after_step>', 'periodic;<interval>', 'logarithmic;<scale>']")})
